...
load ValidationData % preload validation data for following calculation
...
tic;
beta = 10/60;% heating rate in K/s
...
% Here considering the limited fitting performance of these candidate reaction models in the whole pyrolysis region, partitioning the pyrolysis domain to improve the fitting precision has been designed.
Indice_lb = find(ValidationData(:, 1) > 200 +273, 1, 'first'); % divide the temperature domain towards the lower bound side
Indice_ub = find(ValidationData(:, 1) < 700 +273, 1, 'last'); % divide the temperature domain towards the upper bound side
T = ValidationData(Indice_lb:Indice_ub,1);
% T = 401:5:1250;% pyrolysis temperature domain interested. For example, '298', '10', and '1273' denotes the starting temperature (K), incremental step (K), and final temperature (K), respectively
...
plot(T-273, sol_alpha); title(['Simulated TG curve with ' num2str(length(sol_alpha)) ' kinetic models']); xlabel('Pyrolysis temperature/^oC'); ylabel('The degree of conversion \alpha');
...
%%  Step 4 Calculate weight w in accordance with w = 1 - (1 - w_inf)*alpha
tic;
...
plot(T-273, w_T, ValidationData(Indice_lb:Indice_ub,1)-273, ValidationData(Indice_lb:Indice_ub,2),'ro','MarkerSize', 5);
title(['Simulated TG curve with ' num2str(length(w_T)) ' kinetic models']); 
xlabel('Pyrolysis temperature/^oC'); ylabel('Normalized weight/%');

figure('Name', 'Separate TG curves simulated') % for separately overseeing simulated TG curves with root mean square error (RMSE)
% Calculate RMSE between the calculated values and experimental ones from literature for validation
RMSE = []; % pre-allocate storage space for speed
for i = 1 : length(w_T(end, :))
    RMSE(:,i) = sqrt(sum((w_T(:,i) - ValidationData(Indice_lb:Indice_ub,2)).^2)./length(w_T(:, end)));
end
...
for i = 1 : length(w_T(end, :))
    subplot(13, 8, i); 
    plot(T-273, w_T(:,i), ValidationData(Indice_lb:Indice_ub,1)-273, ValidationData(Indice_lb:Indice_ub,2),'ro','MarkerSize', 1.5); 
    title(['G(\alpha) ' num2str(i) ' with RMSE ' num2str(RMSE(i))]);
end
% Plot the fitting performance of the optimal model identified
figure
plot(T-273, w_T(:,OptimalFunctionIndice), ValidationData(Indice_lb:Indice_ub,1)-273, ValidationData(Indice_lb:Indice_ub,2),'ro','MarkerSize', 1.5);
title(['G(\alpha) ' num2str(OptimalFunctionIndice) ' with RMSE ' num2str(RMSE(OptimalFunctionIndice))]);
time_step(:, 4) = toc;
fprintf(['The time elapsed in Step 4 was ' num2str(time_step(:, 4)) ' seconds.\n']) % show time elapsed in this step
 %% Step 5 Calculate external heat required to drive the occurence of thermochemical reactions during pyrolysis
tic;
...
figure
plot(T-273, Q_required, 'ro');title('External energy required', 'FontSize', 14); 
xlabel('Pyrolysis temperature/^oC', 'FontSize', 12); ylabel(' Electricty required for pyrolysis process/(kWh/kg dried sludge)', 'FontSize', 12);
time_step(:, 5) = toc;
fprintf(['The time elapsed in Step 5 was ' num2str(time_step(:, 5)) ' seconds.\n']) % show time elapsed in this step
