...
% Variables0 = [Location VolatileMatters FixedCarbon Ash C H N O S Degree_conversion];
...
input = Variables'; % In Variables,  the name of each column was named by the order of {'Location' 'VolatileMatters/%'...
 ...
 % 'Ash_SiO2/(mg/g)' 'Ash_Na2O/(mg/g)' 'Ash_MgO/(mg/g)' 'Ash_Al2O3/(mg/g)' 'Ash_K2O/(mg/g)' 'Ash_CaO/(mg/g)'...
 % 'Ash_P2O5/(mg/g)' 'Ash_CuO/(mg/g)' 'Ash_ZnO/(mg/g)' 'Ash_Fe2O3/(mg/g)'...
 % 'Degree of conversion'}
target = Target.Variables';

%% Settings of bp-DNN architecture
hiddenLayer = [27, 27, 27, 27, 27];
net = nncreate(hiddenLayer);

% Training parameters
net.trainParam.mc = 0.87; % Optimal net.trainParam.mc = 0.97
net.trainParam.lr = 0.75; % A large learing rate (lr) could accelerate the learning speed for convergence. 0.75 for reference
...
legend({'Observed E_a/(kJ/mol)', 'Predicted E_a/(kJ/mol)'}, 'Location','northwest'); xlabel('Index in dataset'); ylabel('Output in training');
...
if validation
    ...
    legend({'Observed E_a/(kJ/mol)', 'Predicted E_a/(kJ/mol)'}, 'Location','northwest'); xlabel('Index in dataset'); ylabel('Outputin in validation');
end
...
if testing
    ...
    legend({'Observed E_a/(kJ/mol)', 'Predicted E_a/(kJ/mol)'}, 'Location','northwest'); xlabel('Index in dataset'); ylabel('Output in testing');
end
 ...
 title(['Overall performance R^2 = ' num2str(Rsq_overall)]); xlabel('Observed values in E_a/(kJ/mol)'); ylabel('Predicted values in E_a/(kJ/mol)');
 ...
 title(['Training performance R^2 = ' num2str(Rsq_training)]); xlabel('Observed values in E_a/(kJ/mol)'); ylabel('Predicted values in E_a/(kJ/mol)');
...
% For validation case
 ...
 title(['Validation performance R^2 = ' num2str(Rsq_validation)]); xlabel('Observed values in E_a/(kJ/mol)'); ylabel('Predicted values in E_a/(kJ/mol)');
 ...
 title(['Testing performance R^2 = ' num2str(Rsq_testing)]); xlabel('Observed values in E_a/(kJ/mol)'); ylabel('Predicted values in E_a/(kJ/mol)');
...
for j = 1:length(FigList)
  ...
  print(FigHandle, fullfile(FolderName, [FigName '.tif']), '-dtiff', '-r600');
  % Also save as EPS for publication quality
  print(FigHandle, fullfile(FolderName, [FigName '.eps']), '-depsc2', '-r600');
end
%% 

save("Results_trained.mat");% save the output data results after running
zip('Result_needed.zip', {'Results_trained.mat','Figures', 'bpDNN4Ea.m'});
% rmdir Figures s
