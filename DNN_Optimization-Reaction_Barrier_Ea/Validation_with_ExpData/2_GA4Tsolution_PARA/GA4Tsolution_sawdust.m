...
% Ea_bpDNN = SampleOut_WoodSawdust; % Predicted activation energy in J/mol
...
%%
for j = 1 : length(Ea_bpDNN)
    Eam(j) = Ea_bpDNN(j);
    for i =  1 : size(G_alpha, 2)
         Ea = Eam(j);
         fitnessFcn = @(X) (Integral4Tintegral(X, Ea, beta) - G_alpha(j, i)).^2;
         lb1 = [293 2.5E18];
         [T(i, :, j), fval(i, j)] = ga(fitnessFcn, nvars, A, b, Aeq, beq, lb1, ub, nonlcon, options);
    end
end
...
for i = 1 : size(G_alpha, 2)
    ...
    lb10 = [293 2.5E18];
    [T9(i, :), fval9(i)] = ga(fitnessFcn10, nvars, A, b, Aeq, beq, lb10, ub, nonlcon, options);
end
