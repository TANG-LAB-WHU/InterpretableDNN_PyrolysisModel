...
def search_best_combinations(
    ...
    """Brute-force search for the best multi-feedstock combinations.

    Parameters
    ----------
    product_name : str
        Target product ("Biochar", "Bioliquid" or "Biogas").
    candidate_ids : list[int]
        List of feedstock IDs considered in the search space.
    feat_names : List[str]
        Ordered feature names used by the model.
    combo_size : int
        Exact number of feedstocks to include in each combination.
    ratio_vals : np.ndarray
        Grid of candidate mixing-ratio values to test for each feedstock.
    ratio_limit : float
        Upper bound on the sum of mixing ratios in a combination.

    Returns
    -------
    pd.DataFrame
        Sorted dataframe of the best combinations found.
    """
    ...
    # Pre-retrieve column indices for efficiency
    ...
    for combo in itertools.combinations(candidate_ids, r):
        # Exhaustively iterate over all ratio combinations (Cartesian product)
        ratio_iter = itertools.product(ratio_vals, repeat=r)

        for ratios in ratio_iter:
            total_ratio = sum(ratios)
            if total_ratio <= 0 or total_ratio > ratio_limit:
                continue  # skip invalid ratio sets
            ...
            # Record every combination, even those with negative improvements, so that
            # the output CSV always contains the best-performing strategies. A positive
            # filter will be applied later when sorting by Delta_yield_% if desired.
            records.append(
                {
    ...
    df = pd.DataFrame(records)
    # Sort descending by gain for easier interpretation
    df = df.sort_values("Delta_yield_%", ascending=False).reset_index(drop=True)
    return df
...
def process_product(product_name: str, shap_root: str, out_root: str, top_n: int):
    ...
    if _SIMULATE and _COMBO_SIZE >= 2:
        ...
        if len(candidate_ids) < _COMBO_SIZE:
            logger.info("Not enough candidates (%d) for combo size %d – skipping", len(candidate_ids), _COMBO_SIZE)
        else:
            ...
            if not combo_df.empty:
                ...
                logger.info("Saved %d combination strategies → %s", len(combo_df), combo_csv)
            else:
                logger.info("No positive-gain combinations found for %s", product_name)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Exact number of feedstocks to include in each combination strategy (default: %(default)s)",
    )
    ...
    if _TEST_RATIO is None:
        try:
            ...
            raise ValueError("--ratio_search must be in 'start:end:step' format with 0 <= start < end <= 1")
