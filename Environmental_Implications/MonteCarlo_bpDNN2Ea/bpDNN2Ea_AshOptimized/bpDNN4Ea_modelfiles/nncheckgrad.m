...
%NNCHECKGRAD performs validation check for gradients but does not modify 
%   any parameters of the network.
...
for k = 1 : Nl
    for i = 1 : size(net.weight{k}, 1)
        for j = 1 : size(net.weight{k}, 2)
            ...
            rand('state', 0) % reset the random number generator to its initial state
