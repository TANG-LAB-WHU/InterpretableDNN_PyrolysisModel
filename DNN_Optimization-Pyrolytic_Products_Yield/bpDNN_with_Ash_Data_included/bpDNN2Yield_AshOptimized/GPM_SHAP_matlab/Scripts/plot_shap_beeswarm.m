...
% Define target product names (even if not a multi-target model)
productNames = {'Biochar', 'Bioliquid', 'Biogas'};
...
if ~exist('shapValues', 'var') || ~exist('input', 'var') || ~exist('featureNames', 'var')
    ...
    if exist(shapFile, 'file')
        ...
        varsNeeded = {'shapValues', 'input', 'featureNames', 'targetNames'};
        for i = 1:length(varsNeeded)
            if isfield(loadedData, varsNeeded{i})
                ...
                elseif strcmp(varsNeeded{i}, 'targetNames') && ~isfield(loadedData, 'targetNames')
                    % Create default target names
                    if ndims(loadedData.shapValues) == 3
                        numTargets = size(loadedData.shapValues, 3);
                        defaultTargetNames = cell(1, numTargets);
                        for t = 1:numTargets
                            if t <= length(productNames)
                                defaultTargetNames{t} = sprintf('Target %d: %s', t, productNames{t});
                            else
                                defaultTargetNames{t} = sprintf('Target %d', t);
                            end
                        end
                    else
                        defaultTargetNames = {'Target 1: Biochar'};
                    end
                    assignin('base', 'targetNames', defaultTargetNames);
                    fprintf('Created default target names with product descriptions\n');
                end
            end
        end
    else
...
if ndims(shapValues) == 3
    ...
    % Calculate feature importance using Excel-style logic:
    % 1. First calculate mean absolute SHAP for each feature per target
    % 2. Then average across targets
    importanceMatrix = zeros(numFeatures, numTargets);
    for t = 1:numTargets
        importanceMatrix(:, t) = mean(abs(shapValues(:,:,t)), 1)';
    end
    
    % Calculate overall importance - average across all targets
    % This matches the 'Overall' column in SHAP_summary_all_targets.xlsx
    featureImportance = mean(importanceMatrix, 2)';
    
    % For visualization, we need to convert the 3D values to 2D
    % Use WEIGHTED average based on feature importance per target
    shapValues2D = zeros(numInstances, numFeatures);
    
    for i = 1:numFeatures
        % Get importance of this feature across all targets
        featureImportanceAcrossTargets = importanceMatrix(i, :);
        
        % Normalize to get weights that sum to 1
        targetWeights = featureImportanceAcrossTargets / sum(featureImportanceAcrossTargets);
        
        % Apply weighted average for this feature across all samples
        for s = 1:numInstances
            featureValues = squeeze(shapValues(s, i, :))';
            shapValues2D(s, i) = sum(featureValues .* targetWeights);
        end
    end
    ...
    % For 2D SHAP values (already for a single target), just compute regular importance
...
% Check if targetNames variable exists
if ~exist('targetNames', 'var')
    % Create default targetNames if not available
    if ndims(shapValues) == 3
        numTargets = size(shapValues, 3);
        targetNames = cell(1, numTargets);
        for t = 1:numTargets
            if t <= length(productNames)
                targetNames{t} = sprintf('Target %d: %s', t, productNames{t});
            else
                targetNames{t} = sprintf('Target %d', t);
            end
        end
    else
        targetNames = {'Target 1: Biochar'};
    end
    fprintf('Using default target names with product descriptions\n');
end

% Generate descriptive titles and filenames with product information
if ndims(shapValues) == 3
    productSuffix = ' (Combined Targets)';
    targetDesc = '';
    for t = 1:length(targetNames)
        if t > 1
            targetDesc = [targetDesc, ', '];
        end
        [~, productName] = strtok(targetNames{t}, ':');
        if ~isempty(productName)
            targetDesc = [targetDesc, strtrim(productName(2:end))];
        else
            targetDesc = [targetDesc, targetNames{t}];
        end
    end
    productSuffix = [' (' targetDesc ')'];
else
    % For single target model, use its name
    [~, productName] = strtok(targetNames{1}, ':');
    if ~isempty(productName)
        productSuffix = [' (' strtrim(productName(2:end)) ')'];
    else
        productSuffix = '';
    end
end

% 1. Create a plot showing top features
topFeaturesTitle = ['SHAP Values Impact - Top ' num2str(numFeaturesToShow) ' Features' productSuffix];
topFeaturesName = 'beeswarm_top_features';
createBeeswarmPlot(shapValues, sortOrder, sortedFeatureNames, min(numFeaturesToShow, numFeatures), input, topFeaturesTitle, topFeaturesName, figDir);

% 2. Create a plot showing only significant features (above mean importance)
if numSignificantFeatures > 0
    sigFeaturesTitle = ['SHAP Values Impact - ' num2str(numSignificantFeatures) ' Significant Features' productSuffix];
    sigFeaturesName = 'beeswarm_significant_features';
    createBeeswarmPlot(shapValues, significantFeatureOrder, significantFeatureNames, numSignificantFeatures, input, sigFeaturesTitle, sigFeaturesName, figDir);
else
    warning('No significant features found above the threshold of %.4f', significantFeatureThreshold);
end

% 3. Create a plot showing all features (if requested and not too many)
if numFeatures <= 50 % Only create for manageable number of features
    allFeaturesTitle = ['SHAP Values Impact - All ' num2str(numFeatures) ' Features' productSuffix];
    allFeaturesName = 'beeswarm_all_features';
    createBeeswarmPlot(shapValues, sortOrder, sortedFeatureNames, numFeatures, input, allFeaturesTitle, allFeaturesName, figDir);
else
    disp(['Skipping full feature plot due to high feature count. Consider using plot_shap_summary.m instead.']);
end

% 4. Create a standalone colorbar legend for reference
createColorbarLegend(figDir);

% Information to console
fprintf('Created beeswarm plots in: %s\n', figDir);
...
function createBeeswarmPlot(shapValues, featureOrder, featureNames, numFeaturesToPlot, input, figTitle, figName, figDir)
    ...
    % Remove duplicate colorbar definitions and use just one
    cb = colorbar;
    colormap(cmap);
    ...
    % Save the plot with high resolution PNG (avoid duplicate commands)
...
function createColorbarLegend(figDir)
    ...
    % Save the legend (avoid duplicate commands)
