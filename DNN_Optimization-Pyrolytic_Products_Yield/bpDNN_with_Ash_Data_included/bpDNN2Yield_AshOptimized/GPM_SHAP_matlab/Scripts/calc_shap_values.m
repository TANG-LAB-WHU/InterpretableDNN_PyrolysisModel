...
varNames = {'Location', 'VolatileMatters/%', 'FixedCarbon/%', 'Ash/%', ...
            'C/%', 'H/%', 'N/%', 'O/%', 'S/%', ...
            'Ash_SiO2/%', 'Ash_Na2O/%', 'Ash_MgO/%', 'Ash_Al2O3/%', ...
            'Ash_K2O/%', 'Ash_CaO/%', 'Ash_P2O5/%', 'Ash_CuO/%', ...
            'Ash_ZnO/%', 'Ash_Fe2O3/%', ...
            'Temperature/°C', 'ReactionTime/min', 'HeatingRate/(K/min)', 'ReactorType'};
...
% Create target names with product descriptions
targetNames = cell(1, numTargets);
productNames = {'Biochar', 'Bioliquid', 'Biogas'};
for i = 1:numTargets
    if i <= length(productNames)
        targetNames{i} = sprintf('Target %d: %s', i, productNames{i});
    else
        targetNames{i} = sprintf('Target %d', i);
    end
end
fprintf('Created target names with product descriptions: %s\n', strjoin(targetNames, ', '));
    ...
    if exist('PS_global', 'var') && exist('TS_global', 'var')
        ...
        if exist('predictions', 'var') && ~isempty(predictions)
            save(fullfile(dataDir, 'shap_results.mat'), 'shapValues', 'baseValue', 'varNames', ...
                'input', 'target', 'X_background', 'predictions', 'PS_global', 'TS_global', 'targetNames');
            fprintf('Saved SHAP results with predictions, global variables, and target names\n');
        else
            save(fullfile(dataDir, 'shap_results.mat'), 'shapValues', 'baseValue', 'varNames', ...
                'input', 'target', 'X_background', 'PS_global', 'TS_global', 'targetNames');
            fprintf('Saved SHAP results with global variables and target names (no predictions)\n');
        end
    else
        if exist('predictions', 'var') && ~isempty(predictions)
            save(fullfile(dataDir, 'shap_results.mat'), 'shapValues', 'baseValue', 'varNames', ...
                'input', 'target', 'X_background', 'predictions', 'targetNames');
            fprintf('Saved SHAP results with predictions and target names (no global variables)\n');
        else
            save(fullfile(dataDir, 'shap_results.mat'), 'shapValues', 'baseValue', 'varNames', ...
                'input', 'target', 'X_background', 'targetNames');
            fprintf('Saved SHAP results with target names (no predictions or global variables)\n');
        end
    end
catch ME
