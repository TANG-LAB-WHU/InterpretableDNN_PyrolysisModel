...
% Create target names if not loaded
...
if ndims(shapValues) == 3
    ...
    % Create a combined target description for title
    targetDesc = '';
    ...
    if ~isempty(productName)
        targetSuffix = [' (' strtrim(productName(2:end)) ')'];
    else
        targetSuffix = '';
    end
end
...
title(['Feature Impact on Model Output', targetSuffix], 'FontSize', 16, 'FontWeight', 'bold');
...
% Create a file name with target information
if ndims(shapValues) == 3
    % For multi-target, create a file with combined description
    fileName = 'shap_summary_combined';
else
    % Get product name for filename
    [~, productName] = strtok(targetNames{1}, ':');
    if ~isempty(productName)
        productNameClean = strtrim(productName(2:end));
        % Replace spaces with underscores for filename
        productNameClean = strrep(productNameClean, ' ', '_');
        fileName = ['shap_summary_' productNameClean];
    else
...
hold off;
print(gcf, fullfile(figDir, [fileName '.png']), '-dpng', '-r600');
saveas(gcf, fullfile(figDir, [fileName '.fig']));
